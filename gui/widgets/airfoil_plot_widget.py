...
class AirfoilPlotWidget(pg.PlotWidget):
    ...
    def plot_airfoil(
        ...
        COLOR_ORIGINAL_DATA = (100, 149, 237, 220)  # CornflowerBlue - more visible than SkyBlue

        COLOR_TE_TANGENT_UPPER = pg.mkPen((220, 20, 60), width=2, style=Qt.PenStyle.SolidLine)  # Crimson
        COLOR_TE_TANGENT_LOWER = pg.mkPen((138, 43, 226), width=2, style=Qt.PenStyle.SolidLine)  # BlueViolet

        # Comb colours - improved visibility
        COLOR_BSPLINE_COMB = pg.mkPen((150, 150, 150), width=1.5)  # Slightly darker gray
        COLOR_COMB_OUTLINE = pg.mkPen((255, 215, 0), width=2, style=Qt.PenStyle.DotLine)  # Gold

        # B-spline colors - improved scheme
        COLOR_BSPLINE_CURVE = pg.mkPen((255, 0, 127), width=2.5)  # Deep Pink - more vibrant
        COLOR_BSPLINE_CURVE_ALT = pg.mkPen((0, 191, 255), width=2.5)  # Deep Sky Blue - better contrast
        COLOR_BSPLINE_CONTROL_POINTS = pg.mkPen((255, 20, 147), width=1.5, style=Qt.PenStyle.DashLine)  # Deep Pink
        COLOR_BSPLINE_CONTROL_SYMBOL = pg.mkBrush((255, 20, 147, 220))  # Slightly more opaque
        COLOR_BSPLINE_CONTROL_SYMBOL_PEN = pg.mkPen((255, 20, 147), width=1.5)
        
        # Thickened B-spline colors - improved visibility
        COLOR_THICKENED_BSPLINE_CURVE = pg.mkPen((199, 21, 133), width=3.0)  # Medium Violet Red
        COLOR_THICKENED_BSPLINE_CURVE_ALT = pg.mkPen((0, 139, 139), width=3.0)  # Dark Cyan
        COLOR_THICKENED_BSPLINE_CONTROL_POINTS = pg.mkPen((148, 0, 211), width=1.5, style=Qt.PenStyle.DotLine)  # Dark Violet
        COLOR_THICKENED_BSPLINE_CONTROL_SYMBOL = pg.mkBrush((148, 0, 211, 220))
        COLOR_THICKENED_BSPLINE_CONTROL_SYMBOL_PEN = pg.mkPen((148, 0, 211), width=1.5)

        # Knot marker colors - improved visibility
        COLOR_KNOT_MARKER_PEN = pg.mkPen((255, 215, 0), width=1.5)  # Gold
        COLOR_KNOT_MARKER_BRUSH = pg.mkBrush((255, 215, 0, 220))  # Gold with opacity
        ...
        if bspline_upper_curve is not None or bspline_lower_curve is not None:
            ...
            if bspline_is_blunt:
                curve_pens = [COLOR_THICKENED_BSPLINE_CURVE, COLOR_THICKENED_BSPLINE_CURVE_ALT]
                ...
                curve_pens = [COLOR_BSPLINE_CURVE, COLOR_BSPLINE_CURVE_ALT]
            ...
            for curve, surface_name in [(bspline_upper_curve, "Upper"), (bspline_lower_curve, "Lower")]:
                ...
                for i in range(len(unique_knots) - 1):
                    ...
                    pen = curve_pens[i % 2]
                    # Only add name to the first span of each surface for the legend
                    name = f"{curve_name_prefix} {surface_name}" if i == 0 else None
                ...
                # Mark knot points on the curve
                knot_points = curve(unique_knots)
                self.plot_items["B-spline Knot Markers"].append(
                    self.plot(
                        knot_points[:, 0],
                        knot_points[:, 1],
                        pen=None,
                        symbol="o",
                        symbolSize=4,
                        symbolBrush=COLOR_KNOT_MARKER_BRUSH,
                        symbolPen=COLOR_KNOT_MARKER_PEN,
        ...
        # B-spline error text with control points count
        if chord_length_mm is not None and (max_bspline_upper is not None or max_bspline_lower is not None):
            ...
            # Add control points count if available
            cp_info = ""
            if bspline_num_cp_upper is not None and bspline_num_cp_lower is not None:
                if bspline_num_cp_upper == bspline_num_cp_lower:
                    cp_info = f"Control Points: {bspline_num_cp_upper}<br/>"
                else:
                    cp_info = f"Control Points: Upper={bspline_num_cp_upper}, Lower={bspline_num_cp_lower}<br/>"
            elif bspline_num_cp_upper is not None:
                cp_info = f"Control Points (Upper): {bspline_num_cp_upper}<br/>"
            elif bspline_num_cp_lower is not None:
                cp_info = f"Control Points (Lower): {bspline_num_cp_lower}<br/>"
